...
- Deleting a note or removing an inline embed goes through `NotesArea`'s own undo queue + `UndoToastStack` (no native `confirm` for single notes; notebooks keep it). Muted small text uses the `--text-muted` token (WCAG ≥4.5:1 both themes) — never `opacity` on text.
